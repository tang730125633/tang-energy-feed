...
#
# This script runs the end-to-end Content Delivery Workflow:
#   0. Already-sent-today check (Q3b run-level dedup)
#   1. Load Config
#   2. Fetch Feed
#   3. Check Content (abort if feed is empty)
#   4. Classify Candidates (with seen-urls dedup filter, Q3b article-level)
#   5. AI Remix
#   6. Build Card + Deliver
#   7. Archive (Q2 + Q3a: write 3 files, update seen-urls, write .last-sent-date)
#
# Called by launchd/cron at 10:30 and 11:00 daily, or manually for testing.
# The 11:00 call is a backup — if the 10:30 run succeeded, Step 0 makes
# the 11:00 run a no-op.
#
...
#
# Exit codes:
#   0 = success (report sent, archived, last-sent-date updated)
#        OR already sent today (idempotent skip)
...
#
# FORCING A RESEND: delete .last-sent-date and re-run. Use with care —
# this will bypass the dedup check and potentially re-post the same
# articles if seen-urls.json was not updated.
...
# -----------------------------------------------------------------------------
# Step 0: Already-sent-today check (Q3b run-level dedup)
# -----------------------------------------------------------------------------
# If today's digest has already been sent successfully, exit immediately
# and happily. This is what makes the 10:30 + 11:00 double-trigger safe.
echo ""
echo "▸ Step 0/7: Already-sent-today check"
TODAY=$(today_local)
...
# -----------------------------------------------------------------------------
# Step 4: Classify Candidates (auto-loads archive/seen-urls.json)
# -----------------------------------------------------------------------------
echo ""
echo "▸ Step 4/7: Classify candidates"
...
# archive.py is designed to always exit 0 except for truly broken input.
echo ""
echo "▸ Step 7/7: Archive"
